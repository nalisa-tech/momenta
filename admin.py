...
from .models import Category, Event, Booking, EventGallery, PaymentTransaction
...
class EventGalleryAdmin(admin.ModelAdmin):
    list_display = ("media_preview", "event_name", "media_type_badge", "caption_short", "live_performance_badge", "order", "edit_link")
    list_filter = ("event", "media_type", "is_live_performance")
    ...
    fieldsets = (
        ("📋 Basic Information", {
            "fields": ("event", "media_type", "caption", "order")
        }),
        ("🖼️ Image Upload", {
            "fields": ("image",),
            "classes": ("collapse",),
        ...
        ("🎥 Video Configuration", {
            "fields": ("video_url", "video_thumbnail", "is_live_performance"),
            "classes": ("collapse",),
            "description": "Add YouTube/Vimeo video URL for live performances (recommended for music events)"
        }),
    )
    
    def media_preview(self, obj):
        if obj.media_type == 'video' and obj.video_url:
            # Show video thumbnail or default video icon
            if obj.video_thumbnail:
                ...
                return format_html(
                    '<div style="width: 80px; height: 80px; background: linear-gradient(135deg, #ff6b6b, #ee5a24); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white;">'
    ...
    def media_type_badge(self, obj):
        if obj.media_type == 'video':
            return format_html(
                '<span style="background: #ff6b6b; color: white; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: bold;">'
                '🎥 VIDEO'
                '</span>'
            )
        else:
            return format_html(
                '<span style="background: #4ecdc4; color: white; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: bold;">'
                '🖼️ IMAGE'
                '</span>'
            )
    media_type_badge.short_description = "Type"

    def live_performance_badge(self, obj):
        if obj.is_live_performance:
            return format_html(
                '<span style="background: #feca57; color: white; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: bold;">'
                '🎤 LIVE'
                '</span>'
            )
        return format_html('<span style="color: #999; font-size: 11px;">-</span>')
    live_performance_badge.short_description = "Live Performance"
